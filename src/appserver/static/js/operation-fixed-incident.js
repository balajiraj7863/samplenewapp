...
function loadIncidentStatusChart(chartType, rows) {
    ...
    if ("tillDate" === chartType || "today" === chartType) {
        ...
        processStackBarData(rows, opsIncPostDrilldown.weekly.statuswise, "statuswiseChart", fixedIncidentStatus, "txndate", "status", "totalincidents");
    } else if ("monthly" === chartType) {
        processStackBarData(rows, opsIncPostDrilldown.monthly.statuswise, "statuswiseChart", fixedIncidentStatus, "txndate", "status", "totalincidents");
    } else if ("customDate" === chartType) {
        processStackBarData(rows, opsIncPostDrilldown.customDate.statuswise, "statuswiseChart", fixedIncidentStatus, "txndate", "status", "totalincidents");
    }
}
...
function IncidentOnclick(system) {
    ...
    if ($("#fromDate").val() != "" && $("#toDate").val() != "") {
        if ($("#fromDate").val().indexOf(":0:0:0") == -1) {
          var fromDate = $("#fromDate").val() + ':0:0:0';
        }
