       ...
       // highchartsInitialize();
                    ...
                    function () {
                        if (isNoData) {
                            Highcharts
                                .chart(
                                    'orderPlanChart',
                                    bizFixedOrderEmpty.emptyChartConfig.emptyChart);
                        }
                    }, 3000);
            ...
            countryWiseSearchData.on('search:done', function () {
                ...
                orderCountryChartSearchResults.on("data", function (queryResults) {
                    if (undefined != queryResults.data()) {
                        ...
                        // loadCountryChart(chartType, queryResults.data().rows);
                        isNoData = false;
                    }
                });
                setTimeout(
            ...
            if (chartType != "today") {
                ...
                orderTrendData.on('search:done', function () {
                    ...
                    orderTrendSearchDataResults.on("data", function (queryResults) {
                        if (undefined != queryResults.data()) {
                            Highcharts.chart('orderTrendChart', bizFixedOrderEmpty.emptyChartConfig.emptyChart);
                            var trendChartdata;
                            isNoData = false;
                            var finalDataJson = convertResultToJSON(queryResults.data());
                            if (chartType == "daily") {
                                trendChartdata= trendDataProcessor(finalDataJson.data, "txndate", "totalorders");
                                bizFixedOrder.daily.intakeTrend.xAxis[0].categories = trendChartdata.xaxisValues;
                                bizFixedOrder.daily.intakeTrend.series[0].data = trendChartdata.currentWeekData;
                                bizFixedOrder.daily.intakeTrend.series[1].data = trendChartdata.previousWeekData;
                                Highcharts.chart('orderTrendChart', bizFixedOrder.daily.intakeTrend);
                            } else {
                                trendChartdata= trendWithoutComparison(finalDataJson.data, "txndate", "totalorders");
                                bizFixedOrder.weekly.intakeTrend.xAxis[0].categories = trendChartdata.axis;
                                bizFixedOrder.weekly.intakeTrend.series[0].data = trendChartdata.data;
                                Highcharts.chart('orderTrendChart', bizFixedOrder.weekly.intakeTrend);
                            }
        ...
        function loadPlanChart(chartType, chartData) {
            if (chartType == "today") {
                ...
                bizFixedOrder.today.planwise.series[0].data = othersdata;
                Highcharts.chart('orderPlanChart',
                    bizFixedOrder.today.planwise);
            } else {
               
                    processStackBarData(chartData,  bizFixedOrder[chartType].planwise, "orderPlanChart","", "txndate", "tariff_dec", "totalorders");
                 
            }
        }

        function loadCountryChart(chartType, chartData) {
            Highcharts.chart('orderCountryChart', bizFixedOrderEmpty.emptyChartConfig.emptyChart);
            if (chartType == "today") {
                countryWiseChart = Highcharts.chart('orderCountryChart', bizFixedOrder.today.countrywise);
                setPieData(chartData,countryWiseChart, "country", "totalorders");
                //setPieChartData(rows, countryWiseChart);
            } else {
                countryWiseChart = bizFixedOrder.daily.countrywise;
        ...
        function loadStatusChart(chartType, queryResults) {
            Highcharts.chart('orderStatusChart', bizFixedOrderEmpty.emptyChartConfig.emptyChart);
            ...
            statusWiseChart = bizFixedOrder.today.statuswise;
