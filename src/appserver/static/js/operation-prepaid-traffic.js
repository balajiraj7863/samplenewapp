...
require([
    ...
    function (
        ...
        updateHelpText('opsPreRealTimeTrafficHelp');
        ...
        $('#customDate').click(function () {
            ...
            //update the HelpTextIcon
            updateHelpText('opsPreDailyTrafficHelp');
            disableCustomDatePicker(false, 'fromTrafficDate', 'toTrafficDate');
            $("#toTrafficDate").attr("disabled", false);
            var dailyFromDate=new Date();
            var dailyToDate=new Date();

            dailyFromDate=dailyFromDate.setDate((dailyFromDate.getDate()-8));
            dailyToDate=dailyToDate.setDate((dailyToDate.getDate()-1));
            dailyFromDate=new Date(dailyFromDate);
            dailyFromDate = (dailyFromDate.getMonth() + 1) + "/" + dailyFromDate.getDate() + "/" + dailyFromDate.getFullYear();
            dailyToDate=new Date(dailyToDate);
            dailyToDate = (dailyToDate.getMonth() + 1) + "/" + dailyToDate.getDate() + "/" + dailyToDate.getFullYear();
            $("#fromTrafficDate").val(dailyFromDate);
            $("#toTrafficDate").val(dailyToDate);
        ...
        function processTrafficAreaData(rows, chartID, hour, currWeek, lastWeek) {
            ...
            if (null !== kpiResultData) {
                for (var i = 0; i < kpiResultData.length; i++) {
                    var curtempKpi = [];
                    var prevtempKpi = [];
                    curtempKpi.push(kpiResultData[i][hour], Number(parseFloat(kpiResultData[i][currWeek]).toFixed(2)));
                    prevtempKpi.push(kpiResultData[i][hour], Number(parseFloat(kpiResultData[i][lastWeek]).toFixed(2)));
